    ...
    <div className="max-w-7xl mx-auto px-4 lg:px-8 py-12">
      ...
      {cart.length === 0 ? (
        <div className="text-center py-20 max-w-md mx-auto">
          ...
          <Link to="/products" className="inline-block mt-6 px-8 py-3 rounded-full bg-primary text-primary-foreground hover:bg-rose hover:text-white transition">Shop now</Link>
        </div>
      ) : (
        <div className="grid lg:grid-cols-[1fr_360px] gap-10">
          <div className="space-y-4">
            {cart.map((item) => (
              <div key={item.product.id} className="flex gap-4 p-4 rounded-xl border border-border bg-card">
                <img src={item.product.image_link} alt="" className="size-28 rounded-lg object-cover bg-muted" />
                <div className="flex-1 min-w-0">
                  <p className="text-xs uppercase text-muted-foreground">{item.product.brand}</p>
                  <Link to={`/product/${item.product.id}`} className="font-medium hover:text-rose">{item.product.name}</Link>
                  <p className="mt-2 font-semibold">${Number(item.product.price).toFixed(2)}</p>
                  <div className="flex items-center gap-3 mt-3">
                    <div className="flex items-center border border-border rounded-full">
                      <button onClick={() => updateQty(item.product.id, item.quantity - 1)} className="size-8 grid place-items-center"><Minus className="size-3" /></button>
                      <span className="w-8 text-center text-sm">{item.quantity}</span>
                      <button onClick={() => updateQty(item.product.id, item.quantity + 1)} className="size-8 grid place-items-center"><Plus className="size-3" /></button>
                    </div>
                    <button onClick={() => removeFromCart(item.product.id)} className="text-sm text-muted-foreground hover:text-destructive inline-flex items-center gap-1"><Trash2 className="size-4" /> Remove</button>
                  </div>
                </div>
          ...
          <aside className="bg-muted/40 rounded-2xl p-6 h-fit lg:sticky lg:top-24 space-y-4">
            ...
            <Link to="/checkout" className="block text-center w-full py-3 rounded-full bg-primary text-primary-foreground hover:bg-rose hover:text-white transition">Proceed to Checkout</Link>
          </aside>
        </div>
      )}
    </div>
