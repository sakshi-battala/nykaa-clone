  ...
  return (
    <div className="max-w-7xl mx-auto px-4 lg:px-8 py-10">
      <nav className="text-sm text-muted-foreground mb-6">
        ...
        /{" "}
        <Link to="/products" className="hover:text-rose">
          Shop
        </Link>{" "}
        / <span>{labelForType(product.product_type)}</span>
      </nav>
      ...
      <div className="grid md:grid-cols-2 gap-10 lg:gap-16">
        ...
        <div>
          <div className="flex items-center gap-2">
            {product.brand && (
              <span className="inline-block text-xs uppercase tracking-widest px-2.5 py-1 rounded-full bg-blush text-foreground font-medium">
                {product.brand}
              </span>
            )}
            {product.product_type && (
              <span className="text-xs uppercase tracking-widest px-2.5 py-1 rounded-full bg-muted">
          ...
          <div className="mt-5">
            ...
            {longDesc && (
              <button
                ...
                className="mt-2 text-sm text-rose inline-flex items-center gap-1 hover:underline"
              >
            ...
            <div className="mt-6">
              <p className="text-sm font-medium mb-2">
                ...
                <span className="text-muted-foreground font-normal">
                  {product.product_colors[selectedColor]?.colour_name ||
                    "Select"}
                </span>
              </p>
                ...
                {product.product_colors.slice(0, 12).map((c, i) => (
                  <button
                    ...
                    className={cn(
                      ...
                      selectedColor === i
                        ? "border-rose ring-2 ring-rose/30 ring-offset-2 ring-offset-background"
          ...
          {product.tag_list && product.tag_list.length > 0 && (
            <div className="flex flex-wrap gap-2 mt-6">
              {product.tag_list.slice(0, 6).map((t) => (
                <span
                  key={t}
                  className="px-3 py-1 text-xs rounded-full bg-muted capitalize"
                >
                  {t.replace(/-/g, " ")}
                </span>
              ))}
          ...
          <div className="grid grid-cols-3 gap-3 mt-8 pt-8 border-t border-border text-xs">
            <div className="flex items-center gap-2">
              <Truck className="size-4 text-rose" /> Free delivery
            </div>
            <div className="flex items-center gap-2">
              <ShieldCheck className="size-4 text-rose" /> 100% authentic
            </div>
            <div className="flex items-center gap-2">
              <Sparkles className="size-4 text-rose" /> Cruelty-free
            </div>
          </div>
        </div>
      </div>
      ...
      {related.length > 0 && (
        <ProductRow
          ...
          products={related}
          viewAllSearch={product.brand ? { brand: product.brand } : undefined}
        />
      )}
    </div>
  );
